...
static uint8_t x_buffer[25];  // we so far are using only 20 bytes max
...
// --------|-----------------------------------|-------------------|------|
esp_err_t bno055_read_register(bno055_reg_t reg, uint8_t *p_reg_val){
    
    if( !x_bno_dev[BNO055_I2C_PORT].bno_is_open) {
    ...
    i2c_master_write_byte(cmd, (x_bno_dev[BNO055_I2C_PORT].i2c_address << 1) | WRITE_BIT, ACK_CHECK_EN); 
    // send the register address
    i2c_master_write_byte(cmd, reg, ACK_CHECK_EN);   
    i2c_master_start(cmd);  // start condition again
    // device address with read bit
    i2c_master_write_byte(cmd, (x_bno_dev[BNO055_I2C_PORT].i2c_address << 1) | READ_BIT, ACK_CHECK_EN);   
    ...
    esp_err_t err = i2c_master_cmd_begin(BNO055_I2C_PORT, cmd, 1000 / portTICK_RATE_MS);
...
// --------|-----------------------------------|-------------------------|-----------------------|------|
 ...
 esp_err_t bno055_write_register(bno055_reg_t reg, uint8_t reg_val){
...
esp_err_t bno055_read_data(bno055_reg_t start_reg, uint8_t *buffer, uint8_t n_bytes){
...
esp_err_t bno055_open(bno055_config_t * p_bno_conf )
{
    ...
    err = i2c_driver_install(BNO055_I2C_PORT, I2C_MODE_MASTER,
    ...
    x_bno_dev[BNO055_I2C_PORT].i2c_address = p_bno_conf->i2c_address;
    
    // Read BNO055 Chip ID to make sure we have a connection
    x_bno_dev[BNO055_I2C_PORT].bno_is_open = 1; // bno055_read_register() checks this flag
    uint8_t reg_val;
    err = bno055_read_register(BNO055_CHIP_ID_ADDR, & reg_val);
    ...
    err = bno055_write_register(BNO055_OPR_MODE_ADDR, OPERATION_MODE_CONFIG);
    if(err != ESP_OK) goto errExit;
    vTaskDelay(30 / portTICK_RATE_MS);
    ESP_LOGD(TAG, "Set config oper. mode - Ok");

    // Reset
    err=bno055_write_register(BNO055_SYS_TRIGGER_ADDR, 0x20 );
    if(err != ESP_OK) goto errExit;
    ...
    if(p_bno_conf->use_ext_oscillator) {
    	err=bno055_write_register(BNO055_SYS_TRIGGER_ADDR, 0x80);
        if(err != ESP_OK) goto errExit;
    ...
    err=bno055_write_register(BNO055_PWR_MODE_ADDR, POWER_MODE_NORMAL);
    if(err != ESP_OK) goto errExit;
    vTaskDelay(20 / portTICK_RATE_MS);
...
esp_err_t bno055_get_chip_info(bno055_chip_info_t* chip_inf){
    
    memset(chip_inf, 0, sizeof(bno055_chip_info_t));
    
    esp_err_t err = bno055_read_data(BNO055_CHIP_ID_ADDR, x_buffer, 7);
    if( err != ESP_OK ) return err;
...
esp_err_t bno055_set_opmode(bno055_opmode_t mode ){
    ...
    esp_err_t err=bno055_write_register(BNO055_OPR_MODE_ADDR, mode);
    vTaskDelay(30 / portTICK_RATE_MS);
    return err;
}


esp_err_t bno055_get_opmode(bno055_opmode_t * mode ){
    
    uint8_t ui_mode;
    esp_err_t err = bno055_read_register(BNO055_OPR_MODE_ADDR, &ui_mode);
...
esp_err_t bno055_set_ext_crystal_use(bool use_ext ){
    
    bno055_opmode_t mode;
    esp_err_t err = bno055_get_opmode(& mode );
    if( err != ESP_OK ) return err;
    ...
    err=bno055_write_register(BNO055_SYS_TRIGGER_ADDR, reg_val);
    vTaskDelay(10 / portTICK_RATE_MS);
    return err;
}

esp_err_t bno055_get_system_status(uint8_t *system_status){

	esp_err_t err;
	err = bno055_read_register(BNO055_SYS_STAT_ADDR, system_status);

	vTaskDelay(30 / portTICK_RATE_MS);
	return err;
}

esp_err_t bno055_get_self_test_result(uint8_t *self_test_result){

	esp_err_t err;
	err = bno055_read_register(BNO055_SELFTEST_RESULT_ADDR, self_test_result);

	vTaskDelay(30 / portTICK_RATE_MS);
	return err;
}

esp_err_t bno055_get_system_error(uint8_t *system_error){

	esp_err_t err;
	err = bno055_read_register(BNO055_SYS_ERR_ADDR, system_error);

	vTaskDelay(30 / portTICK_RATE_MS);
	return err;
}

esp_err_t bno055_get_temperature(uint8_t* p_temperature){

  esp_err_t err = bno055_read_register(BNO055_TEMP_ADDR, p_temperature);
  return err;
}

esp_err_t bno055_get_calib_status_byte(uint8_t* calib) {
	esp_err_t err;
	err = bno055_read_register(BNO055_CALIB_STAT_ADDR, calib);
    return err;
}

esp_err_t bno055_get_calib_status(uint8_t* sys, uint8_t* gyro, uint8_t* accel, uint8_t* mag) {
    
    uint8_t calib_status;
	esp_err_t err;
	err = bno055_read_register(BNO055_CALIB_STAT_ADDR, &calib_status);
    if( err != ESP_OK ) return err;
...
esp_err_t bno055_get_quaternion(bno055_quaternion_t* quat) {
    ...
    esp_err_t err = bno055_read_data(BNO055_QUATERNION_DATA_W_LSB_ADDR, x_buffer, 8);
    if( err != ESP_OK ) return err;
...
esp_err_t bno055_get_lin_accel(bno055_vec3_t* lin_accel) {

    esp_err_t err = bno055_read_data(BNO055_LINEAR_ACCEL_DATA_X_LSB_ADDR, x_buffer, 6);
    if( err != ESP_OK ) return err;
...
esp_err_t bno055_get_gravity(bno055_vec3_t* gravity){
    esp_err_t err = bno055_read_data(BNO055_GRAVITY_DATA_X_LSB_ADDR, x_buffer, 6);
    if( err != ESP_OK ) return err;
    
    return _bno055_buf_to_gravity(x_buffer, gravity);
}

esp_err_t bno055_get_fusion_data(bno055_quaternion_t* quat, bno055_vec3_t* lin_accel, bno055_vec3_t* gravity){

    esp_err_t err = bno055_read_data(BNO055_QUATERNION_DATA_W_LSB_ADDR, x_buffer, 20);
    if( err != ESP_OK ) return err;
